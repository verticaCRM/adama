...
if($portfolioitem->c_release_status== "Added"){	
	...
	$listing = json_decode($json);
		...
		$data = array(
		'actionDescription'	=>	'Address Request release for '.$listing->name.'<br>Buyer: '.$user->name.'<br>Listing Broker:'.$listing->assignedTo."<br><a href='".get_bloginfo('url').get_permalink($bbcrm_option['bbcrm_loginbar_dataroom'])."?action=release&pid=".$_POST["pid"]."' class='x2-button'>Release address</a>",
...
elseif($isaddressreleased && true==false):
...
echo '<h3><a class="theme-color1-dk" href="'.$permalink .'">'.$listingitem->c_name_generic_c.'</a></h3>' ;
...
<?php if($isaddressreleased && true==false):?>
...
<br />
<a href="<?php echo $permalink;?>" class="theme-background2-dk portfoliobutton" data-id="<?php echo $listingitem->id;?>" ><?php _e('View Listing Details','bbcrm');?></a>
<form method="post" style="display:inline"><input type=hidden name="uid" value="<?php echo $crmid;?>"><input type=hidden name="pid" value="<?php echo $portfolioitem->id;?>"><input type=hidden name="action" value="hide"><input type=submit value="Hide from Data Room" class="theme-background2-dk portfoliobutton"></form> 
